...
if(!NASA_CORE_ACTIVED || (isset($nasa_opt['enable_portfolio']) && !$nasa_opt['enable_portfolio'])) :
    include_once FLOZEN_THEME_PATH . '/404.php';
...
<div class="row">
    <div class="content large-12 columns margin-bottom-50">
        <?php if (have_posts()) :
            ...
            <h3><?php esc_html_e('No pages were found!', 'flozen-theme') ?></h3>
        <?php endif; ?>
        <div class="clear"></div>
        <?php
        ...
        if (function_exists('nasa_get_recent_portfolio') && (!isset($nasa_opt['recent_projects']) || $nasa_opt['recent_projects'])) :
            echo nasa_get_recent_portfolio(8, esc_html__('Recent Works', 'flozen-theme'), array($post->ID));
        endif;
        ?>
    </div>
</div>
