 ...
 */
function flozen_google_fonts_url($font_families = array(), $font_set = array()) {
...
if(!isset($nasa_opt['type_font_select']) || $nasa_opt['type_font_select'] == 'custom') {
    $nasa_custom_font = 'NS-ProximaNova';
     ...
     * Default Custom Font
     */
    if($nasa_custom_font == 'NS-ProximaNova') {
        add_action('wp_enqueue_scripts', 'flozen_custom_fonts_site_default');
        function flozen_custom_fonts_site_default() {
            wp_enqueue_style('nasa-customfonts', FLOZEN_THEME_URI . '/assets/font-nasa-custom/NS-Custom-Font.css');
        }
    }
     ...
     */
        ...
        if(is_file($nasa_upload_dir['basedir'] . '/nasa-custom-fonts/' . $nasa_opt['custom_font'] . '/' . $nasa_opt['custom_font'] . '.css')) {
            ...
            add_action('wp_enqueue_scripts', 'flozen_custom_fonts_site');
            function flozen_custom_fonts_site() {
                global $nasa_opt, $nasa_upload_dir;
                wp_enqueue_style('nasa-customfonts', flozen_remove_protocol($nasa_upload_dir['baseurl']) . '/nasa-custom-fonts/' . $nasa_opt['custom_font'] . '/' . $nasa_opt['custom_font'] . '.css');
            }
        }
    }
}
...
elseif (
    ...
    if (!empty($nasa_font_family) && !empty($nasa_font_set)) {
        ...
        add_action('wp_enqueue_scripts', 'flozen_google_fonts');
        function flozen_google_fonts() {
            global $nasa_font_family, $nasa_font_set;
            
            $font_google_url = flozen_google_fonts_url($nasa_font_family, $nasa_font_set);
            if($font_google_url) {
