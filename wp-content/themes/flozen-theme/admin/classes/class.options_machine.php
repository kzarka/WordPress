...
class Flozen_Options_Machine {
                ...
                switch ($value['type']) {
                    ...
                    case 'typography':
                        ...
                        if (isset($typography_stored['size'])) {
                            $output .= '<div class="select_wrapper typography-size" original-title="' . esc_attr__('Font size', 'flozen-theme') . '">';
                        ...
                        if (isset($typography_stored['face'])) {
                            ...
                            $faces = array(
                                'arial' => esc_html__('Arial', 'flozen-theme'),
                                'verdana' => esc_html__('Verdana, Geneva', 'flozen-theme'),
                                'trebuchet' => esc_html__('Trebuchet', 'flozen-theme'),
                                'georgia' => esc_html__('Georgia', 'flozen-theme'),
                                'times' => esc_html__('Times New Roman', 'flozen-theme'),
                                'tahoma' => esc_html__('Tahoma, Geneva', 'flozen-theme'),
                                'palatino' => esc_html__('Palatino', 'flozen-theme'),
                                'helvetica' => esc_html__('Helvetica', 'flozen-theme')
                            );
                        ...
                        if (isset($typography_stored['style'])) {
                            ...
                            $styles = array(
                                'normal' => esc_html__('Normal', 'flozen-theme'),
                                'italic' => esc_html__('Italic', 'flozen-theme'),
                                'bold' => esc_html__('Bold', 'flozen-theme'),
                                'bold italic' => esc_html__('Bold Italic', 'flozen-theme')
                            );
                    ...
                    case 'border':
                        ...
                        $styles = array(
                            'none' => esc_html__('None', 'flozen-theme'),
                            'solid' => esc_html__('Solid', 'flozen-theme'),
                            'dashed' => esc_html__('Dashed', 'flozen-theme'),
                            'dotted' => esc_html__('Dotted', 'flozen-theme')
                        );
                    ...
                    case "instagram_acc":
                        ...
                        $output .= '<input class="nasa_instagram' . $classInput . '" id="' . $value['id'] . '" type="text" name="' . $value['id'] . '" value="' . $smof_data[$value['id']] . '" placeholder="' . esc_attr__('Enter Your Access Token', 'flozen-theme') . '" />';
                        ...
                        $output .= '<a href="' . $value['url'] . '" class="button-primary nasa-get-intagram' . $classInput . '" title="' . esc_attr__('Get Access Token', 'flozen-theme') . '" target="_blank" style="margin-top: 10px; margin-right: 10px;">' . esc_html__('Get Access Token', 'flozen-theme') . '</a>';
                        $output .= '<a href="javascript:void(0);" class="button-primary nasa-check-intagram' . $classInput . '" title="' . esc_attr__('Check', 'flozen-theme') . '" style="margin-top: 10px; margin-right: 10px;">' . esc_html__('Check', 'flozen-theme') . '</a>';
                        $output .= '<a href="javascript:void(0);" class="button-primary nasa-remove-intagram' . $classRemove . '" title="' . esc_attr__('Remove', 'flozen-theme') . '" style="margin-top: 10px;">' . esc_html__('Remove', 'flozen-theme') . '</a>';
                        $output .= '</div>';
                        $output .= '<div class="clear"></div>';
                        break;
                    ...
                    case 'button':
                        ...
                        $output .= '<img src="' . FLOZEN_THEME_URI . '/admin/assets/images/ajax-loader.gif" style="margin-left: 5px; display: none;" />';
                        break;
                    ...
                    case 'slider':
                        ...
                        if ($count < 2) {
                            $oldorder = 1;
                            $order = 1;
                            $output .= self::optionsframework_slider_function($value['id'], $value['std'], $oldorder, $order);
                        } else {
                            $i = 0;
                        ...
                        $output .= '</ul>';
                        $output .= '<a href="#" class="button slide_add_button">' . esc_html__('Add New Slide', 'flozen-theme') . '</a></div>';
                    ...
                    case 'backup':
                        ...
                        $backup = flozen_of_get_options(FLOZEN_ADMIN_BACKUPS);
                        $init = flozen_of_get_options('smof_init');

                        if (!isset($backup['backup_log'])) {
                            $log = esc_html__('No backups yet.', 'flozen-theme');
                        } else {
                        ...
                        $output .= '<p><strong>' . esc_html__('Last Backup : ', 'flozen-theme') . '<span class="backup-log">' . $log . '</span></strong></p></div>' . "\n";
                        $output .= '<a href="#" id="of_backup_button" class="button" title="' . esc_attr__('Backup Options', 'flozen-theme') . '">' . esc_html__('Backup Options', 'flozen-theme') . '</a>';
                    ...
                    case 'transfer':
                        ...
                        $output .= '<a href="#" id="of_import_button" class="button" title="' . esc_attr__('Import Options', 'flozen-theme') . '">' . esc_html__('Import Options', 'flozen-theme') . '</a>';
                    ...
                    case 'nasa_upload_custom_font' :
                        ...
                        $output .= '<input type="file" name="zip_packet_font" style="width: 230px" /><button style="margin: 0 0 0 5px; width: 100px" class="button-primary" type="submit" name="upload_new_font" value="new_font">' . esc_html__('Upload', 'flozen-theme') . '</button>';
                        $output .= '</form></span>';
                        break;
                    ...
                    case 'switch':
                        ...
                        //Label ON
                        $on = !isset($value['on']) ? esc_html__('On', 'flozen-theme') : $value['on'];

                        //Label OFF
                        $off = !isset($value['off']) ? esc_html__('Off', 'flozen-theme') : $value['off'];
                    ...
                    case "media":
                        ...
                        $output .= self::optionsframework_media_uploader_function($value['id'], $u_val, $value['mod']);
    ...
    public static function optionsframework_media_uploader_function($id, $std, $mod) {
        ...
        if (function_exists('wp_enqueue_media')) {
            $uploader .= '<span class="button media_upload_button" id="' . $id . '">' . esc_html__('Upload', 'flozen-theme') . '</span>';
            ...
            $uploader .= '<span class="button remove-image ' . $hide . '" id="reset_' . $id . '" title="' . $id . '">' . esc_html__('Remove', 'flozen-theme') . '</span>';
        } else {
            $output .= '<p class="upload-notice"><i>' . esc_html__('Upgrade your version of WordPress for full media support.', 'flozen-theme') . '</i></p>';
        }
    ...
    public static function optionsframework_slider_function($id, $std, $oldorder, $order) {
        ...
        if (!empty($val['title'])) {
            ...
            $slider .= '<li><div class="slide_header"><strong>' . esc_html__('Slide', 'flozen-theme') . ' ' . $order . '</strong>';
        }

        $slider .= '<input type="hidden" class="slide of-input order" name="' . $id . '[' . $order . '][order]" id="' . $id . '_' . $order . '_slide_order" value="' . $order . '" />';

        $slider .= '<a class="slide_edit_button" href="#">' . esc_html__('Edit', 'flozen-theme') . '</a></div>';

        $slider .= '<div class="slide_body">';

        $slider .= '<label>' . esc_html__('Title', 'flozen-theme') . '</label>';
        ...
        $slider .= '<label>' . esc_html__('Image URL', 'flozen-theme') . '</label>';
        $slider .= '<input class="upload slide of-input" name="' . $id . '[' . $order . '][url]" id="' . $id . '_' . $order . '_slide_url" value="' . $val['url'] . '" />';

        $slider .= '<div class="upload_button_div"><span class="button media_upload_button" id="' . $id . '_' . $order . '">' . esc_html__('Upload', 'flozen-theme') . '</span>';
        ...
        $slider .= '<span class="button remove-image ' . $hide . '" id="reset_' . $id . '_' . $order . '" title="' . $id . '_' . $order . '">' . esc_html__('Remove', 'flozen-theme') . '</span>';
        $slider .= '</div>' . "\n";
        ...
        $slider .= '</div>';
        $slider .= '<label>' . esc_html__('Link URL (optional)', 'flozen-theme') . '</label>';
        $slider .= '<input class="slide of-input" name="' . $id . '[' . $order . '][link]" id="' . $id . '_' . $order . '_slide_link" value="' . $val['link'] . '" />';

        $slider .= '<label>' . esc_html__('Description (optional)', 'flozen-theme') . '</label>';
