 ...
 * Mobile header
 */
if (!function_exists('flozen_mobile_header')) :

    function flozen_mobile_header() {
        global $nasa_opt;
        ?>
        <div class="row">
                ...
                <table>
                    <tr>
                        ...
                        <td class="nasa-td-mobile-icons">
                            ...
                            $content_icons .= '<div class="nasa-mobile-icons-wrap' . $class_icons_wrap . '">';
                            $content_icons .= $toggle_icon;
                            $content_icons .= flozen_header_icons(false, true, true, true, false);
                            $content_icons .= '</div>';
                            
                            echo flozen_str($content_icons);
                            ?>
                        </td>
                    </tr>
                </table>
 ...
 * Add Block header
 */
if (!function_exists('flozen_block_header')):

    function flozen_block_header() {
        ...
        if ($header_block) {
            echo flozen_get_block($header_block);
        }
    }
 ...
 * Add action header
 */
add_action('init', 'flozen_add_action_header');
if (!function_exists('flozen_add_action_header')):

    function flozen_add_action_header() {
        /* INIT Header site */
        add_action('nasa_get_header_theme', 'flozen_get_header_theme', 10);
        
        /* Header Promotion */
        add_action('nasa_before_header_structure', 'flozen_promotion_recent_post', 1);
        
        /* Header Default */
        add_action('nasa_topbar_header', 'flozen_header_topbar', 10, 1);
        add_action('nasa_header_structure', 'flozen_get_header_structure', 10);
        add_action('nasa_header_structure', 'flozen_block_header', 100);
        
        /* Breadcrumb site */
        add_action('nasa_after_header_structure', 'flozen_get_breadcrumb', 999);
    }
 ...
 */
if (!is_home()) :
    add_action('wp_head', 'flozen_share_meta_head');
    if(!function_exists('flozen_share_meta_head')):
        function flozen_share_meta_head() {
            global $post;
            ?>
...
// ! Header main
// **********************************************************************//
if(!function_exists('flozen_get_header_theme')) :
    function flozen_get_header_theme() {
        global $nasa_opt;
        $file = FLOZEN_CHILD_PATH . '/headers/header-main.php';
        include_once is_file($file) ? $file : FLOZEN_THEME_PATH . '/headers/header-main.php';
    }
endif;
...
// **********************************************************************//
if (!function_exists('flozen_get_header_structure')):

    function flozen_get_header_structure() {
        global $nasa_opt, $wp_query, $nasa_root_term_id;

        $hstructure = isset($nasa_opt['header-type']) ? $nasa_opt['header-type'] : '3';
        $page_id = false;
        ...
        if ($fixed_nav === '') {
            $fixed_nav = (isset($nasa_opt['fixed_nav']) && $nasa_opt['fixed_nav']) ? true : false;
        }
        ...
        if($hstructure == 'nasa-custom') {
            remove_action('nasa_header_structure', 'flozen_block_header', 100);
            ...
            flozen_header_builder($header_slug);
        ...
        if (isset($nasa_opt['nasa_in_mobile']) && $nasa_opt['nasa_in_mobile']) {
            ...
            $vertical = in_array($hstructure, array(3)) ? true : false;
            
            $file = FLOZEN_CHILD_PATH . '/headers/header-mobile.php';
            include is_file($file) ? $file : FLOZEN_THEME_PATH . '/headers/header-mobile.php';
         ...
         * endswitch;
         */
        $show_icon_cat_top = false;
        $show_product_cat = false;
        $show_cart = true;
        $show_compare = true;
        $show_wishlist = true;
        $show_search = in_array($hstructure, array(2, 3)) ? false : true;
        $nasa_header_icons = flozen_header_icons($show_product_cat, $show_cart, $show_compare, $show_wishlist, $show_search);
         ...
         * Main header include
         */
        $file = FLOZEN_CHILD_PATH . '/headers/header-structure-' . ((int) $hstructure) . '.php';
        if (is_file($file)) {
            include $file;
        } else {
            $file = FLOZEN_THEME_PATH . '/headers/header-structure-' . ((int) $hstructure) . '.php';
            include is_file($file) ? $file : FLOZEN_THEME_PATH . '/headers/header-structure-1.php';
        }
    }
...
if (!function_exists('flozen_header_icons')) :
    function flozen_header_icons($product_cat = true, $cart = true, $compare = true, $wishlist = true, $search = true) {
        global $nasa_opt;
        ...
        if(NASA_WOO_ACTIVED && $product_cat) {
            ...
            if($show_icon) {
                $nasa_icon_cat = 
                    '<a class="filter-cat-icon nasa-hide-for-mobile" href="javascript:void(0);" title="' . esc_attr__('Product Categories', 'flozen-theme') . '">' .
                        '<i class="nasa-icon pe-7s-keypad"></i>' .
                    '</a>' .
                    '<a class="filter-cat-icon-mobile" href="javascript:void(0);" title="' . esc_attr__('Product Categories', 'flozen-theme') . '">' .
                        '<i class="nasa-icon pe-7s-keypad"></i>' .
                    '</a>';
                $class = !$first ? 'first ' : '';
                $first = true;
        ...
        if($cart) {
            ...
            $nasa_mini_cart = flozen_mini_cart($show);
            if($nasa_mini_cart != '') {
                $class = !$first ? 'first ' : '';
                $first = true;
        ...
        if($wishlist) {
            $nasa_icon_wishlist = flozen_icon_wishlist();
        ...
        if ($compare && (!isset($nasa_opt['nasa-product-compare']) || $nasa_opt['nasa-product-compare'])) {
            $nasa_icon_compare = flozen_icon_compare();
        ...
        if($search) {
            $search_icon = 
            '<a class="search-icon desk-search" href="javascript:void(0);" data-open="0" title="' . esc_attr__('Search', 'flozen-theme') . '">' .
...
// **********************************************************************//
if(!function_exists('flozen_header_builder')) :
    function flozen_header_builder($header_slug) {
        if(!empty($header_slug)) {
            ...
            $file = FLOZEN_CHILD_PATH . '/headers/header-builder.php';
            include is_file($file) ? $file : FLOZEN_THEME_PATH . '/headers/header-builder.php';
        }
    }
endif;
...
// ! Mobile account menu
// **********************************************************************//
if (!function_exists('flozen_header_topbar')) :

    function flozen_header_topbar($mobile = false) {
        ...
        if (isset($nasa_opt['topbar_content']) && $nasa_opt['topbar_content']) {
            $topbar_content = flozen_get_block($nasa_opt['topbar_content']);
        }
        
        $file = FLOZEN_CHILD_PATH . '/headers/top-bar.php';
        include is_file($file) ? $file : FLOZEN_THEME_PATH . '/headers/top-bar.php';
    }
...
// ! Mobile account menu
// **********************************************************************//
if (!function_exists('flozen_mobile_account')) :

    function flozen_mobile_account() {
        $file = FLOZEN_CHILD_PATH . '/includes/nasa-mobile-account.php';
        include is_file($file) ? $file : FLOZEN_THEME_PATH . '/includes/nasa-mobile-account.php';
    }
