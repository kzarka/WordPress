	...
	class sw_bundle_product_slider_widget extends WP_Widget { 
		...
		**/
		...
		function update( $new_instance, $old_instance ) {
			...
			if ( array_key_exists('category', $new_instance) ){
				if ( is_array($new_instance['category']) ){
					$instance['category'] = array_map( '', $new_instance['category'] );
				} else {
		...
		public function form( $instance ) {
			...
			$columns1     		= isset( $instance['columns1'] )     	? intval($instance['columns1']) : 3;
			$columns2     		= isset( $instance['columns2'] )      	? intval($instance['columns2']) : 3;
			$columns3     		= isset( $instance['columns3'] )      	? intval($instance['columns3']) : 2;
