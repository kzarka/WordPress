...
class Custom_WC_Widget_Layered_Nav extends WC_Widget {
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		// List display.
		echo '<ul class="woocommerce-widget-layered-nav-list custom-filter-layered">';
		...
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				...
				$term_html = '<a rel="nofollow" href="' . esc_url( $link ) . '" title="'. esc_attr( $term->name ) .'"><span '.$attr .'>' . esc_attr( $term->name ) . '<span></a>';
			} else {
				$link      = false;
				$term_html = '<span '.$attr .'>' . esc_attr( $term->name ) . '<span>';
			}
