...
if ( ! function_exists( 'envira_get_gallery_attrs' ) ) {
	 ...
	 * @return void
	 */
	function envira_get_gallery_attrs($atts) {
        ...
        $baseInstance = Envira_Gallery_Lite::get_instance();

        // If no attributes have been passed, the gallery should be pulled from the current post.
        $gallery_id = false;
        if ( empty( $atts ) ) {
            $gallery_id = $post->ID;
            $data       = is_preview() ? $baseInstance->_get_gallery( $gallery_id ) : $baseInstance->get_gallery( $gallery_id );
        } else if ( isset( $atts['id'] ) ) {
            $gallery_id = (int) $atts['id'];
            $data       = is_preview() ? $baseInstance->_get_gallery( $gallery_id ) : $baseInstance->get_gallery( $gallery_id );
        } else if ( isset( $atts['slug'] ) ) {
            $gallery_id = $atts['slug'];
            $data       = is_preview() ? $baseInstance->_get_gallery_by_slug( $gallery_id ) : $baseInstance->get_gallery_by_slug( $gallery_id );
        } else {
