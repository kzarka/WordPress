...
if ( ! function_exists( 'get_soliloquy_slider' ) ) {
	function get_soliloquy_slider ($atts) {
		global $post;
		...
		if ( empty( $atts ) ) {
			$slider_id = $post->ID;
			$data      = is_preview() ? $base->_get_slider( $slider_id ) : $base->get_slider( $slider_id );
		} elseif ( isset( $atts['id'] ) ) {
			$slider_id = (int) $atts['id'];
			$data      = is_preview() ? $base->_get_slider( $slider_id ) : $base->get_slider( $slider_id );
		} elseif ( isset( $atts['slug'] ) ) {
			$slider_id = $atts['slug'];
			$data      = is_preview() ? $base->_get_slider_by_slug( $slider_id ) : $base->get_slider_by_slug( $slider_id );
		} else {
...
if ( ! function_exists( 'get_soliloquy_config' ) ) {
	function get_soliloquy_config ($name, $data) {
		return Soliloquy_Shortcode_Lite::get_instance()->get_config($name, $data);
	}
}